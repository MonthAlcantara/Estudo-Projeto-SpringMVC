...
public class MusicasController {
	@Autowired // Objetos serão criados pelo Spring em tempo de execução e serão injetados por ele
...
public String adicionar(Model model) {
	...
	model.addAttribute("albuns", repositorioAlbum.findAll());//Além da musica vazia, eu devo passar também uma lista de albuns para o usuario escollher em que album ele quer salvar a musica
	return"musica.adicionar.tiles";
	}
	
@RequestMapping(value="/adicionar", method = RequestMethod.POST)
public String adicionar(Model model, @ModelAttribute("musica")/*Pegando o model atributte e setado ele em nova musica*/ @Valid Musica novaMusica, BindingResult result) {
if(result.hasErrors()) {
...
public String excluir(@PathVariable Long id) {
	...
	musica.setAlbum(null); // Setando null ao albuma da musica. Como setei a musica na entidade Album como orphanRemoval, eu vou poder apagar a musica
	repositorioMusica.delete(musica);//Como ela está orfã de album, posso deletar a musica. A referencia entre album e musica está null.
	return "redirect:/musicas/listar";
